...
// Classic single-scroll UI — activated via iOS Settings > Use Classic view
struct ContentView: View {
    @EnvironmentObject private var service: TranslatorService
    ...
    // Classic UI uses the bare entry UUID string as the favourite key
    func classicKey(_ entry: TranslationEntry) -> String { entry.id.uuidString }

    var displayedHistory: [TranslationEntry] {
        showFavoritesOnly
            ? service.history.filter { service.favoritedIDs.contains(classicKey($0)) }
            : service.history
    }
                        ...
                        LazyVGrid(columns: columns, spacing: 16) {
                            ForEach(displayedHistory) { entry in
                                ClassicResultCard(
                                    ...
                                    isFavorited: service.favoritedIDs.contains(classicKey(entry)),
                                    onTapOriginal: { text in repopulate(text, proxy: proxy) },
                                    onToggleFavorite: { service.toggleFavorite(classicKey(entry)) }
                                )
                            }
                        }
                    } else {
            ...
            HStack(spacing: 6) {
                filterGroup(labelKey: "filter_language") {
                    ForEach(FilterLanguage.allCases) { f in
                        filterChip(emoji: f.emoji, labelKey: f.locKey, on: filterLanguage == f) { filterLanguage = f }
                    }
                }
                ...
                filterGroup(labelKey: "filter_gender") {
                    ForEach(FilterGender.allCases) { f in
                        filterChip(emoji: f.emoji, labelKey: f.locKey, on: filterGender == f) { filterGender = f }
                    }
                }
                Divider().frame(height: 24).background(theme.faded.opacity(0.4))
                filterGroup(labelKey: "filter_role") {
                    ForEach(FilterCategory.allCases) { f in
                        filterChip(emoji: f.emoji, labelKey: f.locKey, on: filterCategory == f) { filterCategory = f }
                    }
                }
            }
    ...
    func filterGroup<Content: View>(labelKey: LocalizedStringKey, @ViewBuilder content: () -> Content) -> some View {
        HStack(spacing: 4) {
            Text(labelKey)
                .font(.custom("Georgia", size: theme.scaled(9))).kerning(1.2)
                .textCase(.uppercase).foregroundColor(theme.faded.opacity(0.7))
            content()
        }
    }
    ...
    func filterChip(emoji: String, labelKey: LocalizedStringKey, on: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 2) { Text(emoji); Text(labelKey) }
                .font(.custom("Georgia", size: theme.scaled(11)))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ForEach(visibleStyles) { style in
                    ...
                    Button(action: {
                        if on { activeStyleIDs.remove(style.id) } else { activeStyleIDs.insert(style.id) }
                    }) {
                        HStack(spacing: 4) {
    ...
    func translate() {
        ...
        Task {
            ...
            if service.errorMessage == nil {
                withAnimation(.spring(response: 0.25, dampingFraction: 0.5)) { translateDidSucceed = true }
...
// MARK: - Classic Result Card

struct ClassicResultCard: View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text("\(style.emoji) \(style.label)")
                    ...
                    .kerning(1.5).textCase(.uppercase).foregroundColor(theme.accent)
                Spacer()
                Button(action: { UIPasteboard.general.string = text }) {
                    Image(systemName: "doc.on.doc")
                        .font(.system(size: theme.scaled(12))).foregroundColor(theme.faded)
                }
            }
            Text(text)
                .font(.custom("Georgia", size: theme.scaled(15)))
